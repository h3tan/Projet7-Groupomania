...
exports.requestSaveComment = async (req, res, next) => {
  try {
    connexion.query(
      `insert into comment (comment_text, date_created, date_updated, id_post, id_user) values (?, now(), now(), ?, ?);`,
...
exports.requestAllCommentsOfPost = async (req, res, next) => {
  try {
    connexion.query(
      `select id_comment, comment_text, comment.date_created, comment.id_user, user.nickname, user.picture
...
exports.requestUpdateComment = async (req, res, next) => {
  try {
    connexion.query(
      `update comment set comment_text = ?, date_updated = now() where id_comment = ?`,
